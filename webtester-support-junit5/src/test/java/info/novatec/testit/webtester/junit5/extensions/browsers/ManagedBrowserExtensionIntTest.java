...
class ManagedBrowserExtensionIntTest {
    ...
    @Test
    @DisplayName("instance level browsers are created once per (nested) test")
    void instanceLevelBrowsersAreOnlyInitializedOncePerTest() throws Exception {
        execute(OnlyOneInstanceBrowserCreated.class);
    }

    @ExtendWith(ManagedBrowserExtension.class)
    @CreateBrowsersUsing(OnlyOneInstanceBrowserCreatedBrowserFactory.class)
    private static class OnlyOneInstanceBrowserCreated {
        ...
        class NestedTestClassOne {
            ...
            @Managed
            Browser nestedBrowser;

            @Test
            void nestedClassTest() {
                assertThat(nestedBrowser).isNotNull();
            }
        ...
        class NestedTestClassTwo {
            ...
            @Test
            void nestedClassTestOne() {
                assertThat(nestedBrowser).isNotNull();
            }

            @Test
            void nestedClassTestTwo() {
                assertThat(nestedBrowser).isNotNull();
            }
        ...
        @AfterAll
        static void onlyOneBrowserCreated() {
            assertThat(BROWSERS_CREATED.get()).isEqualTo(4);
        }

    }

    public static class OnlyOneInstanceBrowserCreatedBrowserFactory extends TestBrowserFactory {

        @Override
        public Browser createBrowser() {
            OnlyOneInstanceBrowserCreated.BROWSERS_CREATED.incrementAndGet();
            return super.createBrowser();
        }
